...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(name, "StarNotaryII");
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('awesome starII', 7, {from: user2});

    await instance.exchangeStars(6, 7, {from: user1});
    let owner1 = await instance.ownerOf.call(6);
    let owner2 = await instance.ownerOf.call(7);
    assert(user1, owner2); // owners changed
    assert(user2, owner1); // owners changed
});
...
it('lets a user transfer a star', async() => {
    ...
    let user1 = accounts[1];
    let user2 = accounts[2];
    await instance.createStar('awesome star', 8, {from: user1});

    await instance.transferStar(user2, 8, {from: user1});
    let owner1 = await instance.ownerOf.call(1);
    assert(user2, owner1); // star bought correctly
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starName = await instance.lookUptokenIdToStarInfo.call(1);
    assert(starName, 'awesome star');
});